declare function _exports(context: any): (path: string, options?: import("../../utils").AbortOptions | undefined) => AsyncIterable<UnixFSEntry>;
export = _exports;
export type CID = import("cids/src/");
export type AbortOptions = {
    /**
     * - A timeout in ms
     */
    timeout?: number | undefined;
    /**
     * - Can be used to cancel any long running requests started as a result of this call
     */
    signal?: AbortSignal | undefined;
};
export type UnixTimeObj = {
    /**
     * - the number of seconds since (positive) or before
     * (negative) the Unix Epoch began
     */
    secs: number;
    /**
     * - the number of nanoseconds since the last full
     * second.
     */
    nsecs?: number | undefined;
};
export type FileType = "file" | "directory";
export type UnixFSEntry = {
    cid: CID;
    name: string;
    mode?: number | undefined;
    mtime?: UnixTimeObj | undefined;
    size: number;
    type: FileType;
};
//# sourceMappingURL=ls.d.ts.map