...
var _FilecoinApi_getId, _FilecoinApi_subscriptions, _FilecoinApi_blockchain;
...
const ChannelClosed = "xrpc.ch.close";
...
class FilecoinApi {
    constructor(blockchain) {
        _FilecoinApi_getId.set(this, (id => () => utils_1.Quantity.from(++id))(0));
        _FilecoinApi_subscriptions.set(this, new Map());
        _FilecoinApi_blockchain.set(this, void 0);
        __classPrivateFieldSet(this, _FilecoinApi_blockchain, blockchain, "f");
    }
    async initialize() {
        await __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").initialize();
    }
    async stop() {
        return await __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").stop();
    }
    /**
    ...
    async "Filecoin.Version"() {
        return new version_1.Version({
            blockDelay: BigInt(__classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").minerEnabled
                ? __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").options.miner.blockTime
                : 0)
        }).serialize();
    }
    /**
    ...
    async "Filecoin.ChainGetGenesis"() {
        const tipset = __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").genesisTipset();
    ...
    async "Filecoin.ChainHead"() {
        const tipset = __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").latestTipset();
        return tipset.serialize();
    }
    /**
    ...
    "Filecoin.ChainNotify"(rpcId) {
        const subscriptionId = __classPrivateFieldGet(this, _FilecoinApi_getId, "f").call(this);
        let promiEvent;
        const currentHead = new head_change_1.HeadChange({
            type: head_change_1.HeadChangeType.HCCurrent,
            val: __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").latestTipset()
        });
        const unsubscribeFromEmittery = __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").on("tipset", (tipset) => {
        ...
        __classPrivateFieldGet(this, _FilecoinApi_subscriptions, "f").set(subscriptionId.toString(), unsubscribe);
        promiEvent.emit("message", {
            type: subscriptions_1.SubscriptionMethod.ChannelUpdated,
     ...
     */
    [(_FilecoinApi_getId = new WeakMap(), _FilecoinApi_subscriptions = new WeakMap(), _FilecoinApi_blockchain = new WeakMap(), ChannelClosed)](subscriptionId) {
        const subscriptions = __classPrivateFieldGet(this, _FilecoinApi_subscriptions, "f");
        const unsubscribe = __classPrivateFieldGet(this, _FilecoinApi_subscriptions, "f").get(subscriptionId);
        if (unsubscribe) {
    ...
    async "Filecoin.ChainGetTipSet"(serializedTipsetKey) {
        await __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").waitForReady();
        const tipset = await __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").getTipsetFromKey(serializedTipsetKey.map(serializedCid => new root_cid_1.RootCID(serializedCid)));
        return tipset.serialize();
    }
    /**
    ...
    async "Filecoin.ChainGetTipSetByHeight"(height, serializedTipsetKey) {
        ...
        if (serializedTipsetKey && Array.isArray(serializedTipsetKey)) {
            tipset = await __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").getTipsetByHeight(height, serializedTipsetKey.map(serializedCid => new root_cid_1.RootCID(serializedCid)));
        }
        else {
            tipset = await __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").getTipsetByHeight(height);
        }
        return tipset.serialize();
    }
    /**
    ...
    async "Filecoin.ChainGetBlock"(serializedBlockCid) {
        ...
        const blockHeader = await __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").blockHeaderManager.get(blockCid.root.value);
        if (!blockHeader) {
    ...
    async "Filecoin.ChainGetBlockMessages"(serializedBlockCid) {
        await __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").waitForReady();
        const blockCid = new root_cid_1.RootCID(serializedBlockCid);
        const blockMessages = await __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").blockMessagesManager.getBlockMessages(blockCid.root);
        if (!blockMessages) {
            throw new Error("Could not find a block for the provided CID");
        }
    ...
    async "Filecoin.ChainGetMessage"(serializedMessageCid) {
        ...
        const signedMessage = await __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").signedMessagesManager.get(blockMessageCid.root.value);
        if (!signedMessage) {
    ...
    async "Filecoin.MpoolGetNonce"(address) {
        await __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").waitForReady();
        const account = await __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").accountManager.getAccount(address);
        const pendingMessagesForAccount = __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").messagePool.filter(queuedMessage => queuedMessage.message.from === address);
    ...
    async "Filecoin.MpoolPush"(signedMessage) {
        const rootCid = await __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").pushSigned(new signed_message_1.SignedMessage(signedMessage));
    ...
    async "Filecoin.MpoolBatchPush"(signedMessages) {
        ...
        for (const signedMessage of signedMessages) {
            const cid = await __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").pushSigned(new signed_message_1.SignedMessage(signedMessage));
            cids.push(cid);
        }
    ...
    async "Filecoin.MpoolPushMessage"(message, spec) {
        const signedMessage = await __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").push(new message_1.Message(message), new message_send_spec_1.MessageSendSpec(spec));
    ...
    async "Filecoin.MpoolClear"(local) {
        await __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").mpoolClear(local);
    }
    /**
    ...
    async "Filecoin.MpoolPending"(tipsetKey) {
        const signedMessages = await __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").mpoolPending();
    ...
    async "Filecoin.ActorAddress"() {
        return __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").miner.value;
    }
    /**
    ...
    async "Filecoin.StateMinerPower"(minerAddress) {
        if (minerAddress === __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").miner.value) {
    ...
    async "Filecoin.WalletDefaultAddress"() {
        await __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").waitForReady();
        const accounts = await __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").accountManager.getControllableAccounts();
    ...
    async "Filecoin.WalletSetDefault"(address) {
        await __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").waitForReady();
        await __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").privateKeyManager.setDefault(address);
    }
    /**
    ...
    async "Filecoin.WalletNew"(keyType) {
        ...
        const account = await __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").createAccount(protocol);
    ...
    async "Filecoin.WalletHas"(address) {
        await __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").waitForReady();
        return await __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").privateKeyManager.hasPrivateKey(address);
    }
    /**
    ...
    async "Filecoin.WalletDelete"(address) {
        await __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").waitForReady();
        await __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").privateKeyManager.deletePrivateKey(address);
    }
    /**
    ...
    async "Filecoin.WalletExport"(address) {
        await __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").waitForReady();
        const privateKey = await __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").privateKeyManager.getPrivateKey(address);
        if (privateKey === null) {
    ...
    async "Filecoin.WalletImport"(serializedKeyInfo) {
        ...
        await __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").privateKeyManager.putPrivateKey(address.value, address.privateKey);
    ...
    async "Filecoin.ClientStartDeal"(serializedProposal) {
        ...
        const proposalRootCid = await __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").startDeal(proposal);
    ...
    async "Filecoin.ClientListDeals"() {
        await __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").waitForReady();
        const deals = await __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").dealInfoManager.getDeals();
    ...
    async "Filecoin.ClientGetDealInfo"(serializedCid) {
        await __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").waitForReady();
        const dealInfo = await __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").dealInfoManager.get(serializedCid["/"]);
        if (dealInfo) {
    ...
    "Filecoin.ClientGetDealUpdates"(rpcId) {
        const subscriptionId = __classPrivateFieldGet(this, _FilecoinApi_getId, "f").call(this);
        let promiEvent;
        const unsubscribeFromEmittery = __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").on("dealUpdate", (deal) => {
            if (promiEvent) {
                promiEvent.emit("message", {
                    type: subscriptions_1.SubscriptionMethod.ChannelUpdated,
    ...
    async "Filecoin.ClientFindData"(rootCid) {
        const remoteOffer = await __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").createQueryOffer(new root_cid_1.RootCID(rootCid));
    ...
    async "Filecoin.ClientHasLocal"(rootCid) {
        return await __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").hasLocal(rootCid["/"]);
    }
    /**
    ...
    async "Filecoin.ClientRetrieve"(retrievalOrder, ref) {
        await __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").retrieve(new retrieval_order_1.RetrievalOrder(retrievalOrder), new file_ref_1.FileRef(ref));
    }
    /**
    ...
    async "Ganache.MineTipset"() {
        await __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").mineTipset();
        const tipset = __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").latestTipset();
        return tipset.serialize();
    }
    /**
    ...
    async "Ganache.EnableMiner"() {
        await __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").enableMiner();
    }
    /**
     * Disables the miner.
     */
    async "Ganache.DisableMiner"() {
        await __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").disableMiner();
    }
    /**
    ...
    async "Ganache.MinerEnabled"() {
        return __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").minerEnabled;
    }
    /**
    ...
    "Ganache.MinerEnabledNotify"(rpcId) {
        const subscriptionId = __classPrivateFieldGet(this, _FilecoinApi_getId, "f").call(this);
        let promiEvent;
        const unsubscribeFromEmittery = __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").on("minerEnabled", (minerEnabled) => {
            if (promiEvent) {
                promiEvent.emit("message", {
                    type: subscriptions_1.SubscriptionMethod.ChannelUpdated,
        ...
        promiEvent.emit("message", {
            type: subscriptions_1.SubscriptionMethod.ChannelUpdated,
            data: [subscriptionId.toString(), __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").minerEnabled]
        });
        return promiEvent;
    }
    /**
    ...
    async "Ganache.GetDealById"(dealId) {
        await __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").waitForReady();
        const deal = await __classPrivateFieldGet(this, _FilecoinApi_blockchain, "f").dealInfoManager.getDealById(dealId);
        if (deal) {
