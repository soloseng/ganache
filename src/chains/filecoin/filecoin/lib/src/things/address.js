...
class Address extends serializable_literal_1.SerializableLiteral {
    constructor(publicAddress, privateKey) {
        super(publicAddress);
        _Address_privateKey.set(this, void 0);
        __classPrivateFieldSet(this, _Address_privateKey, privateKey, "f");
    }
    get config() {
        return {};
    }
    get privateKey() {
        return __classPrivateFieldGet(this, _Address_privateKey, "f");
    }
    get network() {
    ...
    async signProposal(proposal) {
        if (__classPrivateFieldGet(this, _Address_privateKey, "f")) {
            const serialized = proposal.serialize();
            const encoded = borc_1.default.encode(serialized);
            const signature = await bls.sign(encoded, __classPrivateFieldGet(this, _Address_privateKey, "f"));
        ...
        if (__classPrivateFieldGet(this, _Address_privateKey, "f")) {
            switch (this.protocol) {
                case AddressProtocol.BLS: {
                    const signature = await bls.sign(buffer, switchEndianness(__classPrivateFieldGet(this, _Address_privateKey, "f")));
                    return Buffer.from(signature);
                }
                case AddressProtocol.SECP256K1: {
                    const hash = blakejs_1.default.blake2b(buffer, null, 32);
                    const result = secp256k1_1.default.ecdsaSign(hash, Buffer.from(__classPrivateFieldGet(this, _Address_privateKey, "f"), "hex"));
...
_Address_privateKey = new WeakMap();
