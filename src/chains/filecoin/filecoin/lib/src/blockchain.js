...
var _Blockchain_miningLock, _Blockchain_minerEnabled, _Blockchain_messagePoolLock, _Blockchain_miningTimeoutLock, _Blockchain_database;
...
class Blockchain extends emittery_1.default {
    constructor(options) {
        super();
        _Blockchain_miningLock.set(this, void 0);
        _Blockchain_minerEnabled.set(this, void 0);
        _Blockchain_messagePoolLock.set(this, void 0);
        _Blockchain_miningTimeoutLock.set(this, void 0);
        ...
        this.messagePool = [];
        __classPrivateFieldSet(this, _Blockchain_messagePoolLock, new async_sema_1.Sema(1), "f");
        this.ready = false;
        this.stopped = false;
        // Create the IPFS server
        this.ipfsServer = new ipfs_server_1.default(this.options.chain);
        this.miningTimeout = null;
        __classPrivateFieldSet(this, _Blockchain_miningTimeoutLock, new async_sema_1.Sema(1), "f");
        // to prevent us from stopping while mining or mining
        // multiple times simultaneously
        __classPrivateFieldSet(this, _Blockchain_miningLock, new async_sema_1.Sema(1), "f");
        __classPrivateFieldSet(this, _Blockchain_minerEnabled, this.options.miner.mine, "f");
        ...
        __classPrivateFieldSet(this, _Blockchain_database, new database_1.default(options.database), "f");
    }
    get minerEnabled() {
        return __classPrivateFieldGet(this, _Blockchain_minerEnabled, "f");
    }
    // This is primarily used by Ganache UI to support workspaces
    get dbDirectory() {
        return __classPrivateFieldGet(this, _Blockchain_database, "f").directory;
    }
    async initialize() {
        await __classPrivateFieldGet(this, _Blockchain_database, "f").initialize();
        this.blockHeaderManager = await block_header_manager_1.default.initialize(__classPrivateFieldGet(this, _Blockchain_database, "f").blocks);
        this.tipsetManager = await tipset_manager_1.default.initialize(__classPrivateFieldGet(this, _Blockchain_database, "f").tipsets, this.blockHeaderManager);
        this.privateKeyManager = await private_key_manager_1.default.initialize(__classPrivateFieldGet(this, _Blockchain_database, "f").privateKeys);
        this.accountManager = await account_manager_1.default.initialize(__classPrivateFieldGet(this, _Blockchain_database, "f").accounts, this.privateKeyManager, __classPrivateFieldGet(this, _Blockchain_database, "f"));
        this.signedMessagesManager = await message_manager_1.default.initialize(__classPrivateFieldGet(this, _Blockchain_database, "f").signedMessages);
        this.blockMessagesManager = await block_messages_manager_1.default.initialize(__classPrivateFieldGet(this, _Blockchain_database, "f").blockMessages, this.signedMessagesManager);
        this.dealInfoManager = await deal_info_manager_1.default.initialize(__classPrivateFieldGet(this, _Blockchain_database, "f").deals, __classPrivateFieldGet(this, _Blockchain_database, "f").dealExpirations);
        ...
        if (recordedGenesisTipset === null) {
            ...
            await __classPrivateFieldGet(this, _Blockchain_database, "f").db.put("latest-tipset", (0, utils_1.uintToBuffer)(0));
        }
        else {
            this.tipsetManager.earliest = recordedGenesisTipset; // initialize earliest
            const data = await __classPrivateFieldGet(this, _Blockchain_database, "f").db.get("latest-tipset");
            const height = utils_1.Quantity.toNumber(data);
        ...
        await this.ipfsServer.start(__classPrivateFieldGet(this, _Blockchain_database, "f").directory);
    ...
    async stop() {
        ...
        await __classPrivateFieldGet(this, _Blockchain_miningLock, "f").acquire();
        await __classPrivateFieldGet(this, _Blockchain_messagePoolLock, "f").acquire();
        await __classPrivateFieldGet(this, _Blockchain_miningTimeoutLock, "f").acquire();
        ...
        if (__classPrivateFieldGet(this, _Blockchain_database, "f")) {
            await __classPrivateFieldGet(this, _Blockchain_database, "f").close();
        }
    }
    ...
    async intervalMine(mine = true) {
        ...
        (0, utils_1.unref)(this.miningTimeout);
        __classPrivateFieldGet(this, _Blockchain_miningTimeoutLock, "f").release();
    }
    async enableMiner() {
        __classPrivateFieldSet(this, _Blockchain_minerEnabled, true, "f");
    ...
    async disableMiner() {
        __classPrivateFieldSet(this, _Blockchain_minerEnabled, false, "f");
    ...
    async push(message, spec) {
        ...
        (0, gas_1.fillGasInformation)(message, spec);
        try {
            await __classPrivateFieldGet(this, _Blockchain_messagePoolLock, "f").acquire();
            ...
            __classPrivateFieldGet(this, _Blockchain_messagePoolLock, "f").release();
    ...
    async pushSigned(signedMessage, acquireLock = true) {
        const error = await (0, message_1.checkMessage)(signedMessage);
        if (error) {
            throw error;
        }
        try {
        ...
        try {
            ...
            if (nextMessagePool.length > 0) {
                ...
                for (const signedMessage of nextMessagePool) {
                    ...
                    const baseFee = (0, gas_1.getBaseFee)();
                    if (baseFee !== 0) {
                        const successful = await this.accountManager.transferFunds(from, BurntFundsAddress.value, (0, gas_1.getMinerFee)(signedMessage.message));
                        if (!successful) {
                    ...
                    // send mining funds
                    let successful = await this.accountManager.transferFunds(from, this.miner.value, (0, gas_1.getMinerFee)(signedMessage.message));
                    if (!successful) {
                        // While we should have checked this when the message was sent,
                        // we double check here just in case
                        const fromAccount = await this.accountManager.getAccount(from);
                        console.warn(`Could not transfer the mining fees of ${(0, gas_1.getMinerFee)(signedMessage.message)} attoFIL from address ${from} due to lack of funds. ${fromAccount.balance.value} attoFIL available`);
                        continue;
                    }
            ...
            await __classPrivateFieldGet(this, _Blockchain_database, "f").db.put("latest-tipset", (0, utils_1.uintToBuffer)(newTipsetHeight));
            // Advance the state of all deals in process.
            const currentDeals = await this.dealInfoManager.getDeals();
            const inProcessDeals = currentDeals.filter(deal => (0, storage_deal_status_1.dealIsInProcess)(deal.state));
            ...
            __classPrivateFieldGet(this, _Blockchain_miningLock, "f").release();
        }
        catch (e) {
            __classPrivateFieldGet(this, _Blockchain_miningLock, "f").release();
            throw e;
...
_Blockchain_miningLock = new WeakMap(), _Blockchain_minerEnabled = new WeakMap(), _Blockchain_messagePoolLock = new WeakMap(), _Blockchain_miningTimeoutLock = new WeakMap(), _Blockchain_database = new WeakMap();
